...
def test(): 
    ...
    for i in sp500List:
        ...
        ts = TimeSeries(key=API_KEY, output_format='pandas')
        data, _ = ts.get_daily(symbol='AMZN',outputsize='compact')
        lastPrice = data.head(1)['4. close']
        sma20, meta_data = ti.get_sma(symbol='AMZN', interval='daily', time_period=20,series_type='close')
        sma60, _ = ti.get_sma(symbol='AMZN', interval='daily', time_period=60,series_type='close')
        sma120, _ = ti.get_sma(symbol='AMZN', interval='daily', time_period=120,series_type='close')
...
def yFinanceSMA():
    ...
    sma120 = data.tail(20)['Close'].mean()

    res = {"C/S" : lastPrice/sma20, "S/M":sma20/sma60, "M/L":sma60/sma120}
    pprint(res)
